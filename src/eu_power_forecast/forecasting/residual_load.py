"""Residual load from SMARD day-ahead forecasts (MW, hourly)."""
...
def compute_residual_load_mw(
    ...
    """
    Residual load (MW) from TSO day-ahead forecasts::

        residual_load = load_forecast - (wind_forecast + solar_forecast + hydro_forecast)

    Uses an **inner** join on the time index so every term is defined for each row.
    """
    tbl = pd.concat(
        {
...
def residual_load_mw_from_bundle_dir(bundle_dir: str | Path) -> pd.DataFrame:
    """Load standard bundle Parquet files and return ``compute_residual_load_mw``."""
    d = Path(bundle_dir)
